        ...
        if deep:
            deep_trace = join_traces(
                _IterableFromIterator(
                    lambda: (lambda x: iter(tqdm(x, total=1 + len(list(trace.sub_filenames)), leave=False)) if show_tqdm else x)(
                        itertools.chain(
                            [Trace(sub_filenames=[], declarations=trace.declarations)],
                            (
                                load_trace(os.path.join(os.path.dirname(filename), sub_filename), deep=True, use_pydantic_parser=use_pydantic_parser)
